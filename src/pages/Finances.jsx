// Finances.jsx
// Proago CRM — Finances (v2025-08-28 hotfix)
// Year → Month → Week → Day → Recruiter drilldown
// Columns aligned: Shifts | Score | Box2 | Box4 | Wages | Income | Profit
// Profit = Income − (Wages + Bonus) at the lowest (recruiter) level
...
import {
  ...
  fmtISO, fmtUK, startOfWeekMon, weekNumberISO, monthKey, toMoney
} from "../util";

// Local: full month label (global request to use full names)
...
// Local helpers (standalone for clarity)
...
export default function Finances({ history }) {
  ...
  // Income per row using discount/no-discount split
  ...
  // Bonus per row (Box2 commission × multiplier)
  ...
  // Filter rows for selected year, de-duplicate by (recruiterId,dateISO,_rowKey)
  ...
  const summarizeRows = (rows) => {
    ...
    const detail = rows.map(r => {
      ...
      return { ...r, score: sc, income: inc, wages: wag, bonus: bon, profit: prof, b2, b4 };
    });
    detail.forEach(d => {
      shifts += 1;
      score  += d.score;
          ...
          return (
            <div key={ym} className="border rounded-xl overflow-hidden">
              ...
              <div className="flex items-center justify-between px-4 py-2 bg-zinc-50">
                <div className="font-medium">{monthLabelFull(ym)}</div>
                                                    ...
                                                    <tr>
                                                      <td colSpan={9} className="p-0">
                                                        <div className="px-2 pb-3">
                                                          {/* Recruiter shift breakdown (no Hours/Rate columns; Bonus included) */}
                                                          <div className="overflow-x-auto border rounded-lg">
                                                            <table className="min-w-full text-sm">
                                                              <thead className="bg-zinc-50">
                                                                <tr>
